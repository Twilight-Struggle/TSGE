...
### MCTS（モンテカルロ木探索）対応
- **Board::copyForMCTS()**：MCTS用の完全なBoardコピーメソッド。不定情報（相手の手札・ヘッドラインカード）を隠蔽しつつ、PhaseMachineでのシミュレーション実行可能
- **情報隠蔽の仕様**：
  - 相手の手札：カード枚数は維持し内容を`CardEnum::Dummy`に置換
  - ヘッドラインカード：宇宙開発トラック4優位性がない場合のみ隠蔽

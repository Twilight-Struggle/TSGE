...
double MCTSExecutor::simulate(Board board, Side maximizing_side) {
  ...
  while (depth < max_depth) {
    auto [legal_moves, next_side, winner] = PhaseMachine::step(board);
    ...
    // ランダムに手を選択
    auto selected_move = rollout_policy.selectMove(legal_moves);
    if (!selected_move) {
      break;
    }

    PhaseMachine::step(board, std::move(selected_move));
    depth++;
  }
